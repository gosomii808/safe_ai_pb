...
REFERER = "https://data.krx.co.kr/contents/MDC/MDI/mdiLoader/index.cmd?menuId=MDC0201010506"
...
def get_ref_start_date(bas_date: str, days_before: int = 7) -> str:
    """
    PER/PBR/배당수익률 Form Data에 필요한 strtDd 생성.
    화면에서는 trdDd=20260521, strtDd=20260514로 들어갔으므로
    기준일 기준 7일 전으로 설정.
    """
...
# =========================
# 3. 하루치 PER/PBR/배당수익률 다운로드
# =========================

def fetch_index_per_pbr_one_day(bas_date: str) -> pd.DataFrame:
    """
    KRX PER/PBR/배당수익률 데이터를 특정 날짜 기준으로 다운로드한다.
    """
...
def clean_index_per_pbr(df: pd.DataFrame) -> pd.DataFrame:
    """
    PER/PBR/배당수익률 데이터를 프로젝트용 영문 컬럼명으로 정리한다.
    """
...
def collect_index_per_pbr_period(
    start_date: str,
    end_date: str,
    output_filename: str,
):
    ...
    for bas_date in date_range(start_date, end_date):
        try:
            df = fetch_index_per_pbr_one_day(bas_date)
    ...
    result = clean_index_per_pbr(result)
    result = result.drop_duplicates()

    if "index_name" in result.columns and "date" in result.columns:
...
if __name__ == "__main__":
    # 기간 설정
    collect_index_per_pbr_period(
        start_date="20260401",
        end_date="20260430",
        output_filename="krx_per_pbr_2604.csv",
    )