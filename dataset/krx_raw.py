...
# 현재 파일 위치 예시:
# safe_ai_pb/dataset/krx_index_to_csv.py
# 프로젝트 루트:
# safe_ai_pb/
...
def fetch_krx_index_one_day(bas_date: str) -> pd.DataFrame:
    """
    KRX 시리즈 일별시세정보 API에서 특정 날짜의 지수 데이터를 가져온다.
...
def clean_krx_index_data(df: pd.DataFrame) -> pd.DataFrame:
    """
...
def collect_krx_index_period(
    start_date: str,
    end_date: str,
    output_file: str
):
    """
    지정한 기간의 KRX 시리즈 지수 데이터를 수집해 CSV로 저장한다.
    """
    ...
    for bas_date in date_range(start_date, end_date):
        try:
            df = fetch_krx_index_one_day(bas_date)
    ...
    result = clean_krx_index_data(result)
...
if __name__ == "__main__":
    collect_krx_index_period(
        start_date="20260101",
        end_date="20260430",
        output_file="../data/raw/krx/krx_index_4m_2601.csv"
    )