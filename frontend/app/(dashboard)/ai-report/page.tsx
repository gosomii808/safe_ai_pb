...
function gradeColor(grade: string | null): string {
  ...
  if (grade.startsWith("B")) return grade === "B+" ? "text-yellow-600 dark:text-yellow-400" : "text-blue-600 dark:text-blue-400";
...
const riskBadge: Record<AiReportRisk, { label: string; cls: string }> = {
  low: { label: "위험도 낮음", cls: "bg-green-500/20 text-green-600 dark:text-green-400" },
  medium: { label: "위험도 중간", cls: "bg-yellow-500/20 text-yellow-600 dark:text-yellow-400" },
  high: { label: "위험도 높음", cls: "bg-red-500/20 text-red-600 dark:text-red-400" },
};
...
export default function AiReportPage() {
  ...
  if (loading) {
    return (
      <PageShell>
        <CenteredNotice
          icon={<Loader2 className="h-6 w-6 animate-spin text-green-600 dark:text-green-400" />}
      ...
      <section className="rounded-2xl border border-border bg-card p-6">
        ...
        <h2 className="mt-3 text-3xl font-bold text-foreground">
          {summary.totalScore != null ? (
            <>
              포트폴리오 종합 점수:{" "}
              <span className="text-green-600 dark:text-green-400">{summary.totalScore}점</span>
            </>
          ) : (
            summary.headline
          )}
        ...
        {/* 등급 그리드 */}
        {summary.totalScore != null && (
          <div className="mt-6 grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-border bg-muted md:grid-cols-4">
        ...
        {isInsufficient && dataQuality.missing.length > 0 && (
          <p className="mt-4 rounded-lg border border-yellow-500/30 bg-yellow-500/10 px-4 py-3 text-sm text-yellow-600 dark:text-yellow-300">
          ...
          <details className="group">
            ...
            <div className="mt-4 border-t border-border pt-3">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                {rawAssets.map((asset) => (
                  <div key={`${asset.market}-${asset.ticker}`} className="bg-background/50 rounded-xl p-3 border border-border">
                    <p className="text-xs font-semibold text-foreground truncate">{asset.stockName || asset.ticker}</p>
                    <p className="text-[10px] text-muted-foreground mt-0.5">{asset.ticker} · {asset.market}</p>
                    <p className="text-sm font-bold text-green-600 dark:text-green-400 mt-2">{asset.quantity.toLocaleString("ko-KR")} 주</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="mt-6 rounded-2xl border border-border bg-card p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
              ...
              <h3 className="text-xl font-bold text-foreground">단기 흐름 분석</h3>
            </div>
          ...
          <div className="mt-4 flex flex-col md:flex-row gap-6 items-start">
            <div className="flex flex-col items-center justify-center bg-background/50 rounded-2xl p-6 min-w-[120px]">
              <span className="text-sm text-muted-foreground">단기 흐름 점수</span>
              <span className="text-4xl font-extrabold text-green-600 dark:text-green-400 mt-2">{report.trendAnalysis.trendScore}점</span>
            </div>
            <div className="flex-1">
              ...
              {report.trendAnalysis.keyDrivers.length > 0 && (
                <div className="mt-4">
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">핵심 단기 모멘텀 요인</p>
      ...
      {eventStudy && eventStudy.length > 0 && (
        <section className="mt-6 rounded-2xl border border-border bg-card p-6">
          <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
            ...
            <h3 className="text-xl font-bold text-foreground">경제 이벤트 영향 분석 (Event Study)</h3>
          </div>
            ...
            {eventStudy.map((study, idx) => (
              <div key={idx} className="rounded-xl border border-border bg-background/50 p-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border pb-3">
                  <div>
                    <h4 className="text-base font-bold text-foreground">{study.eventTitle}</h4>
                    <p className="text-xs text-muted-foreground mt-0.5">이벤트 발생일: {study.eventDate}</p>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3">
                  <div className="bg-background/50 rounded-lg p-3 text-center border border-border">
                    <span className="text-[10px] text-muted-foreground">포트폴리오 수익률</span>
                    <p className="text-sm font-semibold text-foreground mt-1">{study.portfolioReturn}%</p>
                  </div>
                  <div className="bg-background/50 rounded-lg p-3 text-center border border-border">
                    <span className="text-[10px] text-muted-foreground">시장 지수 수익률</span>
                    <p className="text-sm font-semibold text-foreground mt-1">{study.marketReturn}%</p>
                  </div>
                  <div className="col-span-2 sm:col-span-1 bg-background/50 rounded-lg p-3 text-center border border-border flex flex-col justify-center">
                    <span className="text-[10px] text-muted-foreground">abnormalReturn</span>
                    <p className={`text-sm font-bold mt-1 ${study.cumulativeAbnormalReturn >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
                      {study.cumulativeAbnormalReturn >= 0 ? "+" : ""}{study.cumulativeAbnormalReturn}%
                    </p>
                  </div>
                </div>

                <p className="mt-4 text-xs text-muted-foreground bg-muted rounded-lg px-3 py-2 leading-relaxed">
                  {study.summary}
                </p>
              </div>
            ))}
      ...
      {productRecommendations.length > 0 && (
        <section className="mt-6 rounded-2xl border border-border bg-card p-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xl font-bold text-foreground">추천 투자상품 유형</h3>
          </div>
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-2">
            {productRecommendations.map((p) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <h4 className="text-lg font-bold text-foreground">{p.category}</h4>
                    <span
  ...
  return (
    <div className="mx-auto max-w-5xl px-2 py-2">
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">AI 리포트</h1>
          <p className="mt-1 text-muted-foreground">AI가 분석한 맞춤형 투자 인사이트</p>
        </div>
        {badge && (
          <span className="inline-flex items-center gap-1.5 rounded-full border border-green-500/30 bg-green-500/15 px-3 py-1.5 text-sm font-medium text-green-600 dark:text-green-400">
...
function CenteredNotice({
  ...
  return (
    <div className="flex min-h-[40vh] flex-col items-center justify-center rounded-2xl border border-border bg-card p-10 text-center">
...
function AnalysisCard({ card }: { card: AiReportCard }) {
  ...
  return (
    <section className="rounded-2xl border border-border bg-card p-6">
      ...
      <p className="mt-4 font-semibold text-foreground">{card.summary}</p>

      <ul className="mt-3 space-y-2">
        {card.details.map((d, i) => (
          <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
            <ChevronRight className="mt-0.5 h-4 w-4 shrink-0 text-green-600 dark:text-green-400" />
            <span>{d}</span>
          </li>
        ))}
      </ul>
      ...
      {card.recommendation && (
        <div className="mt-4 flex items-start gap-2 rounded-xl bg-green-500/10 px-4 py-3">
          <Lightbulb className="mt-0.5 h-4 w-4 shrink-0 text-green-600 dark:text-green-400" />
          <p className="text-sm text-green-600 dark:text-green-300">{card.recommendation}</p>
        </div>
      )}
    </section>
  );
